...
class Operacao
{
    ...
    public function login()
    {
        ...
        $this->setSuapToken('20221101110085', 'Destroy17');
        echo "\n----------------------------------\n";
    }
    ...
    public function setUser()
    {
        ...
        //"salvando" os dados
        array_push($this->user, json_decode($res, associative: true)['nome_usual']);
        array_push($this->user, json_decode($res, associative: true)['matricula']);
    }
    ...
    {
        try {
            $response = $this->clientServidor->request('POST', 'http://localhost:8000/api/categories', [
                'json' => [
                    'name' => $name,
                ],
                ['headers' => ['Authorization' => 'Bearer ' . $this->getSuapToken()]]
    ...
    public function itens_names()
    {
        try {
            $response = $this->clientServidor->request(
                'GET',
                'http://localhost:8000/api/itens-names',
                ['headers' => ['Authorization' => 'Bearer ' . $this->getSuapToken()]]
            );
            echo 'Status: ' . $response->getStatusCode();
        } catch (GuzzleException\ClientException $e) {
    ...
    { {
            try {
                $response = $this->clientServidor->request('POST', 'http://localhost:8000/api/itens', [
                    'json' => [
                        'name' => $name,
                        'place' => $place,
                        'categorie' => $categorie,
    ...
    {
        try {
            $response = $this->clientServidor->request('DELETE', 'http://localhost:8000/api/itens', [
                'json' => [
                    'id' => $id,
                ],
                ['headers' => ['Authorization' => 'Bearer ' . $this->getSuapToken()]]
    ...
    {
        try {
            $response = $this->clientServidor->request('GET', 'http://localhost:8000/api/itens/view', [
                'json' => [
                    'id' => $id,
                ],
                ['headers' => ['Authorization' => 'Bearer ' . $this->getSuapToken()]]
    ...
    {
        try {
            $response = $this->clientServidor->request('GET', 'http://localhost:8000/api/itens/refound', [
                'json' => [
                    'id' => $id,
                ],
                ['headers' => ['Authorization' => 'Bearer ' . $this->getSuapToken()]]
